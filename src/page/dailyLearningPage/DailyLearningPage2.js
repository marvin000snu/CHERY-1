...
const DailyLearningPage2 = ({ route, navigation }) => {
  const [API] = useCallback(useAPI(), []);
  const { link, name, length } = route.params;
  const Styled = {
    body: styled.View`
      width: 100%;
      height: ${hp(100) - 70}px;
  ...
  const solveNowHandler = async () => {
    ...
    try {
      ...
      Alert.alert(
        "바로풀기",
        `${moment(data["created_at"]).format("MM월 DD일에 풀던")} ${["", "매일학습", "약점학습", "문항분석"][Number(data.testType)]
        ...
        [
          ...
          },
          {
            ...
            onPress: async () => {
              Alert.alert(
                "기존 시험 정보 삭제",
                `${moment(data["created_at"]).format("MM월 DD일에 풀던")} ${["", "매일학습", "약점학습", "문항분석"][
            ...
            <GlobalStyled.ViewCol style={{ flex: 1 }}>
              ...
              >
                <TouchableWithoutFeedback onPress={solveNowHandler}>
                  <Styled.btn>
                    <Styled.btnText>바로풀기</Styled.btnText>
                  </Styled.btn>
